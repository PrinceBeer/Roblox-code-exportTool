...
-- 收集要导出的文件, 优先级:
--   1. 命令行参数(外部控制): lune run export_scenes.lua -- <文件1> [文件2 ...]
--   2. PLACES 手动指定
--   3. 自动扫描当前目录所有 .rbxl
local function collectPlaces()
	local explicit = {}
	local okProc, proc = pcall(function()
		return require("@lune/process")
	end)
	if okProc and type(proc) == "table" and type(proc.args) == "table" then
		for i = 1, #proc.args do
			local f = proc.args[i]
			if type(f) == "string" and f:match("%.rbxl$") then
				local base = (f:match("([^\\/]+)$") or f):gsub("%.rbxl$", "")
				explicit[#explicit + 1] = { file = f, name = base, label = base }
			end
		end
	end
	if #explicit > 0 then
...
for _, place in ipairs(collectPlaces()) do
	...
	if not okPlace then
		print("[跳过] " .. place.file .. " 导出失败: " .. tostring(errPlace))
		print("提示: 文件读取失败请检查文件名/路径; 其他文件不受影响, 会继续导出。")
	end
end
print("全部完成!")
